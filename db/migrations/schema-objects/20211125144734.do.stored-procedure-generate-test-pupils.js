...
module.exports.generateSql = function () {
  if (process.env.NODE_ENV !== 'production') {
    ...
    --
    -- Create 10 pupils for schools 2,3,4,5 inclusive, and suffix the pupil forename and lastnames with 'aad'
    -- EXEC [mtc_admin].[spGenTestPupils] @numPupils = 10, @schoolIdFrom = 2, @schoolIdTo = 5, @dataSetCode = 'aad';
    --
    CREATE OR ALTER PROCEDURE [mtc_admin].[spGenTestPupils]
        @numPupils INT,
        @schoolIdFrom INT = NULL,
        @schoolIdTo   INT = NULL,
        @dataSetCode  VARCHAR(12) = ''
    AS
    ...
    SELECT
        school.id,
        CONCAT('Pupil', @dataSetCode, CAST(temp30.rowNumber AS VARCHAR)), -- foreName
        CONCAT('School', @dataSetCode, CAST(school.id AS VARCHAR)), -- lastName
