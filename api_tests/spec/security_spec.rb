describe 'Security API spec' do
  context 'GET request to admin home' do
    it 'should return response header' do
      ...
      expect(response.headers['content-security-policy']).to eql "default-src 'self'; script-src 'self' 'unsafe-inline' https://www.google-analytics.com; font-src 'self' data:; style-src 'self'; img-src 'self' https://www.google-analytics.com; connect-src 'self'; object-src 'none'; media-src 'none'; child-src 'none'"
      expect(response.headers['x-content-security-policy']).to eql "default-src 'self'; script-src 'self' 'unsafe-inline' https://www.google-analytics.com; font-src 'self' data:; style-src 'self'; img-src 'self' https://www.google-analytics.com; connect-src 'self'; object-src 'none'; media-src 'none'; child-src 'none'"
      expect(response.headers['x-webkit-csp']).to eql "default-src 'self'; script-src 'self' 'unsafe-inline' https://www.google-analytics.com; font-src 'self' data:; style-src 'self'; img-src 'self' https://www.google-analytics.com; connect-src 'self'; object-src 'none'; media-src 'none'; child-src 'none'"
    end
  end
