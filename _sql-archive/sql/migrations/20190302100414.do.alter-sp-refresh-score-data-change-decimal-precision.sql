ALTER PROCEDURE mtc_admin.spRefreshScoreData @checkWindowId INTEGER = NULL
AS
    BEGIN
      ...
      INSERT INTO mtc_admin.schoolScore (checkWindow_id, school_id, score)
        (
          ...
          p.school_id,
          ISNULL((CAST(SUM(ISNULL(latestPupilCheck.mark, 0)) AS DECIMAL(8, 2)) / NULLIF(COUNT(latestPupilCheck.id), 0)), 0) as schoolScore
          FROM
