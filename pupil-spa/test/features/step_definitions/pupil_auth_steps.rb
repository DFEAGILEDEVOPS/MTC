...
Given(/^I attempt to login whilst the check window is not open$/) do
  original = MongoDbHelper.get_check_window_via_name('Summer 2017')
  @original_date = original['endDate']

  updated = original.each{ |key,str| original['endDate'] = Time.now - 400}
