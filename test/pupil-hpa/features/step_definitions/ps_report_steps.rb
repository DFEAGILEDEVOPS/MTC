Given(/^I have completed the check for the ps report$/) do
  input_type = 'numpad'
...
Given(/^I logged in with user with access arrangement '(.*)' for the ps report$/) do |access_arrangments_type|
  step 'I add a pupil'
  PsReportHelper.store_pupil(@details_hash, @school_id)
...
Given(/^I generated a pin after applying a restart for the ps report$/) do
...
Given(/^I have generated a pin for a pupil for the ps report$/) do
  step 'I add a pupil'
  step 'I login to the admin app'
  PsReportHelper.store_pupil(@details_hash, @school_id, 'ps_report_data_set_2.csv', 'attempt_id_after_login')
  navigate_to_pupil_list_for_pin_gen('live')
  generate_pins_overview_page.generate_pin_using_name(@details_hash[:last_name] + ', ' + @details_hash[:first_name])
  pupil_pin_row = view_and_custom_print_live_check_page.pupil_list.rows.find { |row| row.name.text == @details_hash[:last_name] + ', ' + @details_hash[:first_name] }
  @pupil_credentials = { :school_password => pupil_pin_row.school_password.text, :pin => pupil_pin_row.pin.text }
...
Given(/^I have completed the check for a pupil attending a test school$/) do
  ...
  step 'I have completed the check'
  PsReportHelper.store_pupil(@details_hash, @school_id, 'ps_report_data_set_2.csv', 'test_school')
end

When(/^I have generated a live pin for the ps report$/) do
  step 'I add a pupil'
  step 'I login to the admin app'
  PsReportHelper.store_pupil(@details_hash, @school_id)
  navigate_to_pupil_list_for_pin_gen('live')
...
When(/^I generated a live pin for the ps report$/) do
  step 'I add a pupil'
  step 'I login to the admin app'
  PsReportHelper.store_pupil(@details_hash, @school_id, 'ps_report_data_set_2.csv', 'expired_ntc')
  navigate_to_pupil_list_for_pin_gen('live')
  generate_pins_overview_page.generate_pin_using_name(@details_hash[:last_name] + ', ' + @details_hash[:first_name])
  pupil_pin_row = view_and_custom_print_live_check_page.pupil_list.rows.find { |row| row.name.text == @details_hash[:last_name] + ', ' + @details_hash[:first_name] }
  @pupil_credentials = { :school_password => pupil_pin_row.school_password.text, :pin => pupil_pin_row.pin.text }
...
Given(/^I have completed a check with duplicate questions$/) do
  ...
  step 'I add a pupil'
  step 'I login to the admin app'
  PsReportHelper.store_pupil(@details_hash, @school_id, 'ps_report_data_set_2.csv', 'duplicate')
  navigate_to_pupil_list_for_pin_gen('live')
  generate_pins_overview_page.generate_pin_using_name(@details_hash[:last_name] + ', ' + @details_hash[:first_name])
  pupil_pin_row = view_and_custom_print_live_check_page.pupil_list.rows.find { |row| row.name.text == @details_hash[:last_name] + ', ' + @details_hash[:first_name] }
  @pupil_credentials = { :school_password => pupil_pin_row.school_password.text, :pin => pupil_pin_row.pin.text }
...
Given(/^I have completed the check with only (\d+) correct answers for the ps report$/) do |correct_answers|
...
Given(/^I have completed the check and have an unconsumed restart$/) do
  step 'I have completed the check'
  step 'I login to the admin app'
  PsReportHelper.store_pupil(@details_hash, @school_id, 'ps_report_data_set_2.csv', 'uncomsumed_restart')
  visit ENV["ADMIN_BASE_URL"] + restarts_page.url
  restarts_page.select_pupil_to_restart_btn.click
...
Given(/^I have marked a pupil as not taking check with the (.+) reason for the ps report$/) do |reason|
  @reason = reason
  step 'I add a pupil'
  PsReportHelper.store_pupil(@details_hash, @school_id, 'ps_report_data_set_2.csv', 'attempt_id_null_ntc')
  step 'I login to the admin app'
  visit ENV["ADMIN_BASE_URL"] + pupil_reason_page.url
  pupil_reason_page.select_reason(@reason)
  pupil_row = pupil_reason_page.pupil_list.rows.select { |row| row.name.text.include?(@name) }
...
Given(/^my pin has expired after applying a restart$/) do
  step "I have completed the check"
  step 'I login to the admin app'
  PsReportHelper.store_pupil(@details_hash, @school_id, 'ps_report_data_set_2.csv', 'restart_number_0')
  visit ENV["ADMIN_BASE_URL"] + restarts_page.url
  restarts_page.select_pupil_to_restart_btn.click
  restarts_page.reason_2.click
  pupil = restarts_page.find_pupil_row(@details_hash[:first_name])
...
And(/^the PS report has been triggered$/) do
  visit ENV['ADMIN_BASE_URL'] + '/sign-out'
  visit ENV['ADMIN_BASE_URL']
  admin_sign_in_page.login('tech-support', 'password')
  tech_support_page.run_ps_report.click
  run_ps_report_page.run_ps_report
  p "PS Report triggered at #{Time.now}"
  sleep 1800
end

Then(/^I assert the data in the PS report to ensure its correct$/) do
  pupil_csv = CSV.read(File.expand_path("#{File.dirname(__FILE__)}../../../data/ps_report_data_set_1.csv"))
  pupil_csv.each do |row|
    row = row.compact
    p row
    pupil_details = SqlDbHelper.pupil_details_using_school(row[0], row[1])
    ...
    # expect(ps_report_record["TimeComplete"].strftime("%c")).to eql check_inputs.nil? ? nil : check_inputs.last['inputBrowserTimestamp'].utc.strftime("%c")
    ...
    25.times do |index|
      ...
      expect(ps_report_record["Q#{question}TimeOutResponse"]).to eql (timeout == nil) ? nil : 0
      expect(ps_report_record["Q#{question}TimeOutSco"]).to eql (timeout == nil) ? nil : 0
  ...
  #duplicate scenario
  pupil_csv_2 = CSV.read(File.expand_path("#{File.dirname(__FILE__)}../../../data/ps_report_data_set_2.csv"))
  duplicate_pupil = pupil_csv_2.find { |row| row.include? 'duplicate' }
  school_url = SqlDbHelper.find_school(duplicate_pupil[2])['urlSlug']
  pupil_details = SqlDbHelper.pupil_details(duplicate_pupil[1])
  check_details = SqlDbHelper.check_details(pupil_details['id'])
  @recieved_check = AzureTableHelper.get_row('receivedCheck', school_url, check_details['checkCode'])
  ...
  #test_school
  test_school_pupil = pupil_csv_2.find { |row| row.include? 'test_school' }
  school_urn = SqlDbHelper.find_school(test_school_pupil[2])['urn']
  expect(SqlDbHelper.get_school_records_from_ps_report(school_urn, ps_report_table_name)).to be_empty

  #expired_ntc
  expired_ntc = pupil_csv_2.find { |row| row.include? 'expired_ntc' }
  pupil_details = SqlDbHelper.pupil_details(expired_ntc[1])
  wait_until(ENV['WAIT_TIME'].to_i, 20) { !SqlDbHelper.get_ps_record_for_pupil(ps_report_table_name, pupil_details['id']).nil? }
  ps_report_record = SqlDbHelper.get_ps_record_for_pupil(ps_report_table_name, pupil_details['id'])
  expect(ps_report_record["AttemptId"]).to be_nil

  #uncomsumed_restart
  uncomsumed_restart = pupil_csv_2.find { |row| row.include? 'uncomsumed_restart' }
  pupil_details = SqlDbHelper.pupil_details(uncomsumed_restart[1])
  ps_report_record = SqlDbHelper.get_ps_record_for_pupil(ps_report_table_name, pupil_details['id'])
  expect(ps_report_record['ReasonNotTakingCheck']).to eql 'L'
  expect(ps_report_record['AttemptId']).to be_nil
  expect(ps_report_record['PupilStatus']).to eql 'Not taking the Check'

  #attempt_id_after_login
  attempt_id_after_login = pupil_csv_2.find { |row| row.include? 'attempt_id_after_login' }
  pupil_details = SqlDbHelper.pupil_details(attempt_id_after_login[1])
  ps_report_record = SqlDbHelper.get_ps_record_for_pupil(ps_report_table_name, pupil_details['id'])
  expect(ps_report_record["AttemptId"]).to be_nil
  ...
  #attempt_id_null_ntc
  attempt_id_null_ntc = pupil_csv_2.find { |row| row.include? 'attempt_id_null_ntc' }
  pupil_details = SqlDbHelper.pupil_details(attempt_id_null_ntc[1])
  ps_report_record = SqlDbHelper.get_ps_record_for_pupil(ps_report_table_name, pupil_details['id'])
  expect(ps_report_record["AttemptId"]).to be_nil
  expect(ps_report_record["FormID"]).to be_nil
  expect(ps_report_record["TestDate"]).to be_nil
  ...
  #restart_number_0
  restart_number_0 = pupil_csv_2.find { |row| row.include? 'restart_number_0' }
  pupil_details = SqlDbHelper.pupil_details(restart_number_0[1])
  ps_report_record = SqlDbHelper.get_ps_record_for_pupil(ps_report_table_name, pupil_details['id'])
