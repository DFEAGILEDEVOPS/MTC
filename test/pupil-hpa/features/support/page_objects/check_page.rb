class CheckPage < SitePrism::Page
  ...
  section :number_pad, NumberPadSection, '.numpad-container'
  ...
  def complete_check_with_correct_answers(number_of_questions, input_type, next_arrangement=nil)
    ...
    number_of_questions.to_i.times do
      ...
      wait_until(20, 0.5) { has_question? }
  ...
  def array_of_inputs_from_numpad(array_of_answers, questions)
    ...
    array_of_answers.each_with_index do |answer, index|
      ...
      answer.to_s.chars.map(&:to_i).each do |char|
        @ans << {"input" => char.to_s, "eventType" => "mouse", "question" => questions[index], "sequenceNumber" => index+1}
      end
      @ans << {"input" => "Enter", "eventType" => "mouse", "question" => questions[index], "sequenceNumber" => index+1}
      @inputs_array << @ans
    end
    @inputs_array
  end
  ...
  def array_of_inputs_from_keyboard(array_of_answers, questions)
    ...
    array_of_answers.each_with_index do |answer, index|
      @ans = []
      answer.to_s.chars.map(&:to_i).each do |char|
        @ans << {"input" => char.to_s, "eventType" => "keyboard", "question" => questions[index], "sequenceNumber" => index+1}
      end
      @ans << {"input" => "Enter", "eventType" => "keyboard", "question" => questions[index], "sequenceNumber" => index+1}
      @inputs_array << @ans
    end
    @inputs_array
  end
