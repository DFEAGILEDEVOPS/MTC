class GeneratePinsOverviewPage < SitePrism::Page
  ...
  def generate_pin
    # Needs more thought
    teacher_usernames = ['teacher1']
    teacher_usernames.each do |username|
      ...
      next unless has_pupil_list?
      pupils_with_no_pin = pupil_list.rows.select {|row| row.has_no_selected?}.first
      pupils_with_no_pin.checkbox.click
      @teacher_pupil_hash = {teacher_username: username, pupil_name: pupils_with_no_pin.text}
      sticky_banner.confirm.click
    end
    @teacher_pupil_hash
  end
