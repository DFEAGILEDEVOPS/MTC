...
And(/^we navigate to the Result page$/) do
  ...
  SafeTimeout.timeout(10){visit current_url until school_landing_page.has_results?}
...
Then(/^I should see the provisional results page with a message stating the hdf needs to be signed$/) do
  SafeTimeout.timeout(120) {pupil_register_page.load until REDIS_CLIENT.get("checkWindow.sqlFindActiveCheckWindow") != nil}
  today_date = Date.today
...
Then(/^I should see the school results$/) do
  ...
  SafeTimeout.timeout(120) {sleep 2 until File.exist?(ctf_path)}
...
But(/^I have not signed the hdf$/) do
  ...
  step 'I am logged in'
  school_landing_page.hdf.click
  SafeTimeout.timeout(120) {(visit current_url; sleep 2) until hdf_form_page.has_continue?}
end
