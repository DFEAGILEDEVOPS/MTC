...
Then(/^I should see generate pin familiarisation overview page as per design$/) do
  expect(generate_tio_pins_overview_page).to have_heading
  expect(generate_tio_pins_overview_page).to have_generate_pin_message
  expect(generate_tio_pins_overview_page).to have_access_arrangment_text
  expect(generate_tio_pins_overview_page).to have_access_arrangment_link
  expect(generate_tio_pins_overview_page).to have_generate_pin_btn
end

And(/^I can see instructions for generating pin for familiarisation$/) do
  generate_tio_pins_overview_page.instruction_section.toggle.click
  expect(generate_tio_pins_overview_page.instruction_section.info_message[0].text).to include "Press 'Generate PINs' button."
  expect(generate_tio_pins_overview_page.instruction_section.info_message[1].text).to include "Select which pupils you are generating pins for."
  expect(generate_tio_pins_overview_page.instruction_section.info_message[2].text).to include "Distribute pupil PINs and school password to your pupils."
end
...
Then(/^I should see a list of pupils sorted by surname in '(.*)' order on familiarisation Generate Pins List Page$/) do |sort_order|
  if sort_order.eql?('descending')
    sorted_pupils_from_page = generate_tio_pins_overview_page.pupil_list.rows.map {|x| x.name.text}.sort.reverse
  else
    sorted_pupils_from_page = generate_tio_pins_overview_page.pupil_list.rows.map {|x| x.name.text}.sort
  end
...
Then(/^I should be able to select them via a checkbox on familiarisation Generate Pin page$/) do
  generate_tio_pins_overview_page.pupil_list.rows.each {|pupil| expect(pupil).to have_checkbox}
end

Then(/^I should have a option to select all pupils on familiarisation Generate Pin page$/) do
  expect(generate_tio_pins_overview_page).to have_select_all_pupils
end

When(/^I click on the Pupil heading on familiarisation generate pin pupil list page$/) do
  generate_tio_pins_overview_page.pupil_column_heading.click
end
...
When(/^I select a Pupil from familiarisation Generate Pin page$/) do
  ...
  pupil = generate_tio_pins_overview_page.pupil_list.rows.find {|row| row.has_no_selected?}
  pupil.checkbox.click
end

When(/^I deselect all pupils from familiarisation Generate Pin Page$/) do
  generate_tio_pins_overview_page.select_all_pupils.click
  expect(@page).to have_sticky_banner
  generate_tio_pins_overview_page.select_all_pupils.click
end

When(/^I select multiple pupils from familiarisation Generate Pin Page$/) do
  @pupils = generate_tio_pins_overview_page.pupil_list.rows.select {|row| row.has_no_selected?}
...
Then(/^the familiarisation pin should consist of (\d+) characters$/) do |size|
  expect(view_and_print_tio_pins_page.find_pupil_row(@pupil_name).pin.text.size).to eql size.to_i
end

When(/^I click view all tio pins button$/) do
  generate_tio_pins_overview_page.view_all_pins_btn.click
end
...
Then(/^familiarisation generated pin overview page is displayed as per design$/) do
  expect(generate_tio_pins_overview_page.generated_pin_overview).to be_all_there
end

Then(/^I should be taken to familiarisation pin overview page$/) do
  expect(generate_tio_pins_overview_page).to be_displayed
end

Then(/^all pupil familiarisation pins should be generated from the specified pool of characters$/) do
  pins_array = view_and_print_tio_pins_page.pupil_list.rows.map {|pupil| pupil.pin.text}
...
Then(/^the familiarisation school password should consist of (\d+) characters$/) do |size|
  expect(view_and_print_tio_pins_page.find_pupil_row(@pupil_name).school_password.text.size).to eql size.to_i
end

Then(/^the familiarisation school password should not contain charachter 'q'$/) do
  school_pwd = view_and_print_tio_pins_page.find_pupil_row(@pupil_name).school_password.text
...
When(/^I click Generate tio PINs button$/) do
  generate_tio_pins_overview_page.generate_pins
end
...
When(/^I choose to filter via group on the generate pins familiarisation page$/) do
  ...
  navigate_to_pupil_list_for_pin_gen('tio')
  @page = generate_tio_pins_overview_page
  generate_tio_pins_overview_page.group_filter.closed_filter.click unless generate_live_pins_overview_page.group_filter.has_opened_filter?
  group = generate_tio_pins_overview_page.group_filter.groups.find {|group| group.name.text.include? @group_name}
