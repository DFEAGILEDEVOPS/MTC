...
Given(/^I am on view and custom print for live check page$/) do
  ...
  @page = view_and_print_live_pins_page
end

Then(/^view and custom print page is displayed as per design$/) do
  expect(view_and_print_live_pins_page).to have_heading
  expect(view_and_print_live_pins_page).to have_generate_pin_message
  expect(view_and_print_tio_pins_page).to have_pupil_list
  expect(view_and_print_live_pins_page).to have_select_all_pupils
end

Then(/^I click view all live pins button$/) do
  generate_live_pins_overview_page.generated_pin_overview.view_all_pins_btn.click
end

Then(/^I should be able to select them via a checkbox on Custom Print Live check page$/) do
  view_and_print_live_pins_page.pupil_list.rows.each {|pupil| expect(pupil).to have_checkbox}
end

When(/^I select a Pupil on Custom Print Live check page$/) do
  pupil = view_and_print_live_pins_page.pupil_list.rows.find {|row| row.has_no_selected?}
  pupil.checkbox.click
end

When(/^I deselect all pupils on Custom Print Live check page$/) do
  view_and_print_live_pins_page.select_all_pupils.click
  expect(@page).to have_sticky_banner
  view_and_print_live_pins_page.select_all_pupils.click
end

When(/^I select multiple pupils on Custom Print Live check page$/) do
  @pupils = view_and_print_live_pins_page.pupil_list.rows.select {|row| row.has_no_selected?}
...
When(/^I choose to filter via group on Custom Print Live check page$/) do
  @page = view_and_print_live_pins_page
  view_and_print_live_pins_page.closed_filter.click unless view_and_print_live_pins_page.has_opened_filter?
...
Then(/^I should only see pupils on Custom Print Live check page$/) do
  filtered_pupils = view_and_print_live_pins_page.pupil_list.rows.map {|row| row.name.text.split('Date').first}.compact.map {|pupil| pupil.strip}
