class FunctionsHelper
  ...
  def self.generate_school_pin(school_id)
    HTTParty.post(ENV['FUNC_CONSUMP_BASE_URL'] + "/api/util-school-pin-generator", :body => {'school_id' => school_id}.to_json, headers: {'Content-Type' => 'application/json', 'x-functions-key' => ENV['FUNC_CONSUMP_MASTER_KEY']})
  end
  ...
  def self.create_school(lea_code, estab_code, name, urn)
    HTTParty.put(ENV['FUNC_CONSUMP_BASE_URL'] + "/api/util-create-school", :body => {'leaCode' => lea_code, 'estabCode' => estab_code.to_s, 'name' => name, 'urn' => urn.to_s}.to_json, headers: {'Content-Type' => 'application/json', 'x-functions-key' => ENV['FUNC_CONSUMP_MASTER_KEY']})
  end

  def self.create_user(school_uuid,username)
    HTTParty.put(ENV['FUNC_CONSUMP_BASE_URL'] + "/api/util-create-user", :body => {'schoolUUID' => school_uuid, 'identifier' => username, 'password' => 'password', 'role' => 'teacher'}.to_json, headers: {'Content-Type' => 'application/json', 'x-functions-key' => ENV['FUNC_CONSUMP_MASTER_KEY']})
  end
