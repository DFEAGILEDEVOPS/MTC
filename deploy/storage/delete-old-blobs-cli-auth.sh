...
delete_old_blobs() {
    ...
    blobs=$(az storage blob list --account-name "$STORAGE_ACCOUNT_NAME" --auth-mode login --container-name "$container_name" --query "[?properties.creationTime < '$CUTOFF_DATE'].name" --output tsv)
    ...
    blobs=$(az storage blob list --container-name "$container_name" --account-name "$STORAGE_ACCOUNT_NAME" --auth-mode login --query "[].name" -o tsv)
