...
az group deployment create \
  ...
  --parameters bus_namespace=$BUS_NAME

  # get connection string for consumer apps...
  CONSUMER_CONNECTION_STRING=$(az servicebus namespace authorization-rule keys list --resource-group $RES_GROUP --namespace-name $BUS_NAME --name mtc-consumer | jq .primaryConnectionString)

  # put it in key vault...
  az keyvault secret set --vault-name $KEY_VAULT_NAME --name "ServiceBusConnectionString" --value "$CONSUMER_CONNECTION_STRING"

    # get connection string for consumer apps...
  DEPLOY_CONNECTION_STRING=$(az servicebus namespace authorization-rule keys list --resource-group $RES_GROUP --namespace-name $BUS_NAME --name mtc-deploy | jq .primaryConnectionString)
