...
KEY_INDEX="" # as its an array, key1 is '0' and key2 is '1'
...
KEY_VALUE=$(az storage account keys renew --resource-group $RES_GROUP --account-name $STORAGE_ACCOUNT_NAME --key $STORAGE_ACCOUNT_KEY_TYPE | jq -r .[$KEY_INDEX].$KEY_IDENTIFIER)
