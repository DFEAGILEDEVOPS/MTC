...
function createApp() {
  ...
  plan=$2
  # create web app
  az webapp create -n "$ENV$role-as-$NAME_SUFFIX" -g $RESOURCE_GROUP \
...
az webapp config set -g $RESOURCE_GROUP -n $PUPIL_APP_SVC_PLAN_NAME --always-on true \
  --ftps-state Disabled --http20-enabled true --min-tls-version '1.2' --remote-debugging-enabled false --web-sockets-enabled false
az webapp config set -g $RESOURCE_GROUP -n $AUTH_APP_SVC_PLAN_NAME --always-on true \
  --ftps-state Disabled --http20-enabled true --min-tls-version '1.2' --remote-debugging-enabled false --web-sockets-enabled false
az webapp config set -g $RESOURCE_GROUP -n $ADMIN_APP_SVC_PLAN_NAME --always-on true \
  --ftps-state Disabled --http20-enabled true --min-tls-version '1.2' --remote-debugging-enabled false --web-sockets-enabled false
az webapp config set -g $RESOURCE_GROUP -n $ASSETS_APP_SVC_PLAN_NAME --always-on true \
  --ftps-state Disabled --http20-enabled true --min-tls-version '1.2' --remote-debugging-enabled false --web-sockets-enabled false
