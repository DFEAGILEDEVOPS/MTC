...
SQL_ALLOW_REPLICA_FOR_READS | Bool | false | Optional | AA | Experimental setting 
 ...
 EXPRESS_LOGGING_WINSTON | Boolean | false | Optional | AA | Logging in the Admin app uses either Winston (intended for production) or moran (intended for local developmnent).  Winston log transports can be configured to be sent to Azure, whereas morgan logs to the console. 
 ...
 DFE_SIGNON_DISCOVERY_TIMEOUT_MS | Int | 10,000 | Optional | AA | Custom authentication 
 DFE_SIGNON_OPENID_SCOPE | String | 'openid profile email organisation' | Optional | AA | Custom authentication 
 DFE_USER_INFO_API_URL |  |  |  |  | Custom authentication 
 DFE_USER_INFO_API_SECRET |  |  |  |  | Custom authentication 
 DFE_USER_INFO_API_TOKEN_AUDIENCE |  |  |  |  | Custom authentication 
 DFE_SIGNON_SIGNOUT_URL |  |  |  |  | Custom authentication 
 DFE_SIGNON_SERVICES_URL |  |  |  |  | Custom authentication 
