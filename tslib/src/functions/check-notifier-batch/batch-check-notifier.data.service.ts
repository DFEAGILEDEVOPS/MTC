import { SqlService, type ITransactionRequest, type ISqlParameter } from '../../sql/sql.service.js'
...
export class BatchCheckNotifierDataService implements IBatchCheckNotifierDataService {
  ...
  async createCheckCompleteRequest (checkCode: string): Promise<ITransactionRequest[]> {
    ...
    if (RA.isNonEmptyArray(checkData) === true) {
    ...
    try {
      ...
      if (RA.isNonEmptyArray(pupilData) === true) {
        pupil = pupilData[0]
      }
