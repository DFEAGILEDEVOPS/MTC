...
import R from 'ramda'
import mssql, { IRequestParameters } from 'mssql'
import DateTimeService from '../common/DateTimeService'
import moment from 'moment'
import ILogger from '../common/ILogger'
import retry from './retry-async'
import config from '../config'
import * as RedisService from '../caching/redis-service'
...
const dbLimitReached = (error: SqlError) => {
 ...
 */
const findDataType = (type: string) => Object.keys(sqlService.TYPES).find(k => {
 ...
 * @return {string}
 */
const cacheKey = (table: string, column: string) => table.replace('[', '').replace(']', '') + '_' + column
 ...
 * @type {Function}
 */
const createMultipleParamIdentifiers = (data: any) => data.map((d, idx) => R.compose(
  R.join(' , '),
 ...
 * @return {boolean}
 */
const isMoment = (v: any) => moment.isMoment(v)
 ...
 * Useful for converting Data during UPDATES and INSERTS
 */
const convertDateToMoment = (d: Date | string) => {
  if (
    d instanceof Date ||
 ...
 */
async function generateParams (tableName: string, data: any) {
...
sqlService.generateInsertStatement = async (table, data) => {
  ...
  const sql = `
  INSERT INTO [mtc_admin].${table} ( ${extractColumns(data)} ) VALUES ( ${createParamIdentifiers(data)} );
 ...
 */
sqlService.generateMultipleInsertStatements = async (table: string, data: any) => {
  ...
  // logger.debug('sql.service: Params ', R.compose(R.map(R.pick(['name', 'value'])))(params))
  const sql = `
  INSERT INTO [mtc_admin].${table} ( ${headers} ) VALUES ( ${values} );
 ...
 */
sqlService.generateUpdateStatement = async (table: string, data: any) => {
  const params = await generateParams(table, data)
  const sql = R.join(' ', [
    `UPDATE [mtc_admin].${table}`,
    'SET',
 ...
 */
sqlService.create = async (tableName: string, data: any) => {
 ...
 * @return {Promise<*>}
 */
sqlService.update = async function (tableName: string, data: any) {
  if (!data.id) {
 ...
 */
sqlService.buildParameterList = (ary: [], type: TediousType) => {
  const params = []
...
sqlService.modifyWithTransaction = async (sqlStatements: Array<string>, params: mssql.IRequestParameters) => {
  const wrappedSQL = `
  BEGIN TRY
  BEGIN TRANSACTION
    ${sqlStatements}
