...
describe('CsvTransformer Class', () => {
  beforeEach(() => {
    const psReportLineData: IPsychometricReportLine[] = [
      {
        PupilDatabaseId: 1,
        PupilUPN: 'upn',
  ...
  describe('top level info', () => {
    ...
    test('it outputs the time start', () => {
      const s = sut.transform()
      const res = CSV.parse(s)
      expect(res[0][25]).toBe('2020-06-29T07:49:35.637Z')
    })

    test('it outputs the time complete', () => {
      const s = sut.transform()
      const res = CSV.parse(s)
      expect(res[0][26]).toBe('2020-06-29T07:51:35.637Z')
    })

    test('it outputs the time taken', () => {
      const s = sut.transform()
      const res = CSV.parse(s)
      expect(res[0][27]).toBe('223.45')
    })

    test('it outputs the reason for not taking check', () => {
      const s = sut.transform()
      const res = CSV.parse(s)
      expect(res[0][28]).toBe('L')
    })

    test('it outputs the ToE code', () => {
      const s = sut.transform()
      const res = CSV.parse(s)
      expect(res[0][29]).toBe('99')
    })

    test('it outputs the imported from census flag', () => {
      const s = sut.transform()
      const res = CSV.parse(s)
      expect(res[0][30]).toBe('1')
    })

    test('it outputs the isEdited flag', () => {
      const s = sut.transform()
      const res = CSV.parse(s)
      expect(res[0][31]).toBe('0')
    })
  })
