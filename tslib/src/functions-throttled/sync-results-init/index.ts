...
const timerTrigger: AzureFunction = async function (context: Context): Promise<void> {
  ...
  context.log(`${functionName}: ${timeStamp} processed ${meta.processCount} records, run took ${durationInMilliseconds} ms, total memory usage (heap used) ${Math.round(heapUsed * 100) / 100}MB`)
}
