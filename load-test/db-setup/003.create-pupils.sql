...
DECLARE @leaCode INT;
DECLARE @pupilCount INT = 30;

DECLARE schoolCursor CURSOR FOR
  SELECT id, leaCode FROM [mtc_admin].school

OPEN schoolCursor
FETCH NEXT FROM schoolCursor INTO @schoolId, @leaCode
...
WHILE @@FETCH_STATUS = 0
  BEGIN
    ...
    FROM
      mtc_admin.school s
    INNER JOIN
      (SELECT value as [pupilIndex], @schoolId as [schoolId] FROM GENERATE_SERIES(1, @pupilCount)) as pupilSet;
...
-- END of new code.  delete below when cursor working
