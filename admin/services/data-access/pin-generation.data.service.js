...
const serviceToExport = {
   ...
   *
   * @param {[{pupil_id, checkForm_id, checkWindow_id, isLiveCheck, pinExpiresAtUtc, pinValidFromUtc, school_id}]} checks - array of check objects to create
  ...
  sqlCreateBatch: async (checks) => {
    ...
    const insertHeader = `INSERT into @tvp
        (pupil_id, checkForm_id, checkWindow_id, isLiveCheck, pinExpiresAtUtc, pinValidFromUtc, school_id)
        VALUES`
    const inserts = checks.map((check, index) => {
      return `(@pupilId${index}, @checkFormId${index}, @checkWindowId${index}, @isLiveCheck${index}, @pinExpiresAtUtc${index}, @pinValidFromUtc${index}, @schoolId${index})`
    })
    const params = []
    checks.map((check, index) => {
      ...
      params.push({ name: `pinExpiresAtUtc${index}`, value: check.pinExpiresAtUtc.toDate(), type: TYPES.DateTimeOffset })
      params.push({ name: `pinValidFromUtc${index}`, value: check.pinValidFromUtc.toDate(), type: TYPES.DateTimeOffset })
