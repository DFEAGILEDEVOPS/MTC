...
const serviceToExport = {
   ...
   *
   * @param {[{pupil_id, checkForm_id, checkWindow_id, isLiveCheck, pinExpiresAt, pinValidFrom, school_id}]} checks - array of check objects to create
  ...
  sqlCreateBatch: async (checks) => {
    ...
    const insertHeader = `INSERT into @tvp
        (pupil_id, checkForm_id, checkWindow_id, isLiveCheck, pinExpiresAt, pinValidFrom, school_id)
        VALUES`
    const inserts = checks.map((check, index) => {
      return `(@pupilId${index}, @checkFormId${index}, @checkWindowId${index}, @isLiveCheck${index}, @pinExpiresAt${index}, @pinValidFrom${index}, @schoolId${index})`
    })
    const params = []
    checks.map((check, index) => {
      ...
      params.push({ name: `pinValidFrom${index}`, value: check.pinValidFrom.toDate(), type: TYPES.DateTimeOffset })
