ALTER VIEW [mtc_admin].vewPupilsEligibleForLivePinGeneration AS
  ...
  FROM
       [mtc_admin].[pupil] p
         ...
         LEFT JOIN [mtc_admin].[pupilAttendance] pa ON (p.id = pa.pupil_id AND isDeleted = 0)
         ...
         LEFT JOIN
           (SELECT
            ...
            FROM
                 ...
                 [mtc_admin].[checkStatus] AS chkStatus ON (chk.checkStatus_id = chkStatus.id) LEFT JOIN
                 [mtc_admin].[pupilGroup] g ON (g.pupil_id = p.id)
            WHERE
                chkStatus.code <> 'EXP'
            GROUP BY
                     p.id,
  ...
  FROM
       [mtc_admin].[pupil] p INNER JOIN
       [mtc_admin].[pupilRestart] pr ON (p.id = pr.pupil_id) LEFT JOIN
       [mtc_admin].[pupilGroup] g ON (g.pupil_id = p.id)
  WHERE
