...
FROM [mtc_admin].[pupil] p
         LEFT JOIN
     [mtc_admin].[pupilGroup] pg ON (p.id = pg.pupil_id)
         LEFT JOIN
     [mtc_admin].[group] g ON (pg.group_id = g.id)
         JOIN
     [mtc_admin].[pupilStatus] ps ON (p.pupilStatus_id = ps.id)
         LEFT JOIN
     (
         SELECT *,
...
WHERE (lastCheck.rank = 1 or lastCheck.rank IS NULL)
