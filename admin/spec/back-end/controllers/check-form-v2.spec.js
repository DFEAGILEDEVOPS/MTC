...
describe('check form v2 controller:', () => {
  ...
  describe('postUpload route', () => {
    ...
    it('submits uploaded check form data processing', async () => {
      ...
      const req = getReq(reqParams)
      spyOn(res, 'redirect')
      spyOn(checkFormV2Service, 'processData')
      await controller.postUpload(req, res, next)
      expect(checkFormV2Service.processData).toHaveBeenCalled()
    ...
    it('submits uploaded check form data processing', async () => {
      ...
      spyOn(checkFormV2Service, 'processData').and.returnValue(Promise.reject(error))
      await controller.postUpload(req, res, next)
      expect(checkFormV2Service.processData).toHaveBeenCalled()
      expect(checkFormV2Service.processData).toHaveBeenCalled()
