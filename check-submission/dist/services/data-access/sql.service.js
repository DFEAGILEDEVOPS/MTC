 ...
 * @type {Function}
 */
const extractColumns = R.compose(R.join(' , '), R.keys);
/**
 ...
 * @type {Function}
 */
const createParamIdentifiers = R.compose(R.join(' , '), R.map(paramName), R.keys);
/**
 ...
 * @type {Function}
 */
const generateSetStatements = R.compose(R.join(' , '), R.map(singleUpdate), R.keys);
/**
...
sqlService = {
        ...
        return new Promise(async (resolve, reject) => {
            ...
            if (params) {
                for (let index = 0; index < params.length; index++) {
                    ...
                    request.addParameter(param.name, param.type, param.value, opts);
                }
            }
            // Pick up any OUTPUT
            request.on('row', function (cols) {
     ...
     */
    generateInsertStatement: async (table, data) => {
  ...
  SELECT @@IDENTITY`;
     ...
     */
    generateUpdateStatement: async (table, data) => {
        const params = await generateParams(table, data);
        const sql = R.join(' ', [
     ...
     * @return {Promise<void>}
     */
    updateDataTypeCache: async function () {
        const sql = `SELECT
      TABLE_NAME,
      COLUMN_NAME,
      DATA_TYPE,
    ...
    modifyWithTransaction: async (sqlStatements, params) => {
        const wrappedSQL = `
BEGIN TRY
BEGIN TRANSACTION
  ${sqlStatements}
