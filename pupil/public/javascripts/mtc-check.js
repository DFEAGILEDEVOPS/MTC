(function(global) {
  ...
  GOVUK.clickListener = function () {
    GOVUK.addEvent(document, 'mousedown', function (e) {
      ...
      if (!event.which) {
        /* IE case */
        clickButton = (event.button < 2) ? "left click" :
          ((event.button === 4) ? "middle click" : "right click");
      }
      else {
        /* All others */
        clickButton = (event.which < 2) ? "left click" :
          ((event.which === 2) ? "middle click" : "right click");
      }
